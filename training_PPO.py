...
class PPO(nn.Module):
    ...
    def masked_softmax(self, logits, mask, dim=-1, eps=1e-8):
        ...
        if mask.sum().item() == 0:
            # fallback: no valid actions, uniform over mask shape
            uniform_probs = torch.ones_like(mask)
            return uniform_probs / uniform_probs.sum(dim=dim, keepdim=True)

        logits = logits.masked_fill(mask == 0, float('-inf'))

        if torch.isinf(logits).all():
            fallback_probs = mask / (mask.sum(dim=dim, keepdim=True) + eps)
    ...
    def sample_action(self, state):
        ...
        prob = self.pi(obs, obs_path, mask=valid_mask)
        m = Categorical(prob)
